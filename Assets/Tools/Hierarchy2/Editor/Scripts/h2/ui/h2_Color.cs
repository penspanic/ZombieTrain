...
namespace vietlabs.h2
{
    ...
    internal class h2_Color
    {
        ...
        private static readonly int[] COLOR_INDEXES =
        {
            1, 5, 3, 0, 4, 2
        };
        ...
        public Color GetColor(bool selected = false, bool dark = true, bool focus = true, bool playMode = false)
        {
            ...
            switch (mode)
            {
                case h2_ColorMode.Simple:
                    return colors[0];
                case h2_ColorMode.DarkLight:
                    return colors[dark ? 0 : 1];
                case h2_ColorMode.Advanced:
                    return colors[(dark ? 0 : 1) | ((selected ? focus ? 1 : 2 : 0) << 1)];
            }
