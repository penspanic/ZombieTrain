...
namespace vietlabs.h2
{
    public class h2_Common : h2_Icon
    {
        ...
        protected override void RunCommand(string cmd)
        {
            ...
            switch (cmd)
            {
                ...
                case h2_CommonSetting.CMD_INVERT_SELECT_SIBLING:
                {
                    if (go != null)
                    {
                        var siblings = h2_Unity.GetSiblings(go);
                        ...
                        for (var i = 0; i < oldSelection.Length; i++)
                            d.Add(oldSelection[i]);

                        var result = new List<int>();
                        for (var i = 0; i < siblings.Count; i++)
                        {
                            var instId = siblings[i].GetInstanceID();
                            if (d.Contains(instId)) continue;
                            result.Add(instId);
                        }

                        Selection.instanceIDs = result.ToArray();
                    }
                    return;
                }
    ...
    internal class h2_CommonSetting : h2_FeatureSetting
    {
        ...
        internal const string CMD_INVERT_SELECT_SIBLING = "invert_select_sibling";
        internal const string CMD_SELECT_SCENE = "select_scene";

        private const string TITLE = "GLOBAL SETTINGS";
        private const string DRAW_BG_LABEL = "Icon BG";
        private const string DRAW_SELECTION_LABEL = "Selection BG (experimental)";
        private const string ICON_SPACE_LABEL = "Icon space";
        private const string ICON_OFFSET_LABEL = "Icon offset";
        ...
        private static readonly string[] SHORTCUTS =
        {
            ...
            "Invert Sibling Selection", CMD_INVERT_SELECT_SIBLING, "#2",
