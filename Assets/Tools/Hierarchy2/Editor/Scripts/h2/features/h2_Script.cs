...
namespace vietlabs.h2
{
    public class h2_Script : h2_Icon
    {
        ...
        private static Dictionary<GameObject, h2_ScriptInfo> stateMap;
        private static Dictionary<Type, bool> typeMap;
        ...
        private static h2_ScriptState GetState(GameObject go, float expire)
        {
#if H2_DEV
        ...
        private static bool isValidScript(Component b)
        {
        ...
        private static List<int> AppendMissing(GameObject go, List<int> result)
        {
        ...
        private static void SelectMissingInChildren(GameObject go)
        {
            if (go == null) return;
            var arr = AppendMissing(go, null).ToArray();

            if (arr.Length > 0) Selection.instanceIDs = arr;
            else Debug.Log("No missing component found in children of <" + go.name + ">");
        }

        private static void SelectMissingInScene()
        {
            ...
            if (list.Count > 0) Selection.instanceIDs = list.ToArray();
            else Debug.Log("No missing component found !");
        }
    ...
    internal class h2_ScriptSetting : h2_IconSetting
    {
        ...
        private const string TITLE = "SCRIPT INDICATOR";
