...
namespace vietlabs.h2
{
    ...
    public class Hierarchy2
    {
        ...
        private static int lastMouseDownOn;
        private static bool lastMouseDownSingle;
        private static int firstInst;

        //static float lastRepaintTime;

        private static Dictionary<int, int> widthCache;
        private static int lastDrawIcon;
        private static HashSet<int> _inspectorLocked;

        // ----------------- SETTINGS ----------------------
        //static List<h3_Icon> IconList;
        private static List<h2_Icon> IconList2;

        private static float lastPeriodCheck;
        ...
        private static void PeriodRepaint()
        {
            var dur = h2_Unity.focusingInspector ? 0.1f : 1f;
            if (Time.realtimeSinceStartup - lastPeriodCheck < dur) return;
            lastPeriodCheck = Time.realtimeSinceStartup;

            checkInspectorLock();
            h2_Utils.RepaintHierarchy();
        }

        private static void OnSelectionChange(Object[] gameObjects)
        {
            h2_Utils.DelayRepaintHierarchy();
        }

        private static Rect OnHierarchyDraw(int instID, Rect r)
        {
        ...
        private static void HierarchyItemCB(int instID, Rect r)
        {
            ...
            var isLocked = (_inspectorLocked != null) && _inspectorLocked.Contains(instID);
            ...
            if (h2_Lazy.isRepaint)
            {
                ...
                if (odd)
                {
                    var fullRect = bgRect;
                    fullRect.xMin = 0;
                    h2_GUI.SolidColor(fullRect, new Color(0f, 0f, 0f, 0.05f));
                }
        ...
        private static void checkInspectorLock()
        {
            var inspector = h2_Unity.InspectorWindow;
            if (inspector == null) return;

            if (_inspectorLocked == null)
                _inspectorLocked = new HashSet<int>();
			...
			if (editors.Length > 0) {
				for (var i = 0;i < editors.Length; i++) {
					...
					if (e != null) {
						...
						//return;
						_inspectorLocked.Add(e.target.GetInstanceID());
					}
				}
			}
#else
                ...
                if (editors.Length > 0)
                    for (var i = 0; i < editors.Length; i++)
                    {
                        ...
                        if (ei.target == null) continue;

                        //Debug.Log(i + " locked target : " + ei.target + " ---> " + ei);

                        if (!(ei.target is GameObject)) continue;

                        //_inspectorLocked = editors[i].target.GetInstanceID();
                        //return;
                        _inspectorLocked.Add(ei.target.GetInstanceID());
                    }
#endif
            }
        }
