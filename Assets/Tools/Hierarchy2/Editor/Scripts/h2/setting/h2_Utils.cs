...
namespace vietlabs.h2
{
    internal static class h2_Utils
    {
        ...
        private static readonly Dictionary<string, string> ShortenMap = new Dictionary<string, string>();

        private static bool willRepaintHierarchy;
        private static bool willRepaintInspector;

        private static bool _willSaveAssets;
        ...
        private static void DoSaveAsset()
        {
        ...
        internal static List<SerializedProperty> GetSOArray(SerializedProperty prop, int level)
        {
            var result = new List<SerializedProperty>();
            if (level > 7)
            {
                Debug.Log("Skipping array (too deep) : " + prop.name + ":" + prop.propertyType + ":" +
            ...
            for (var i = 0; i < size; i++)
            {
                ...
                if (p.isArray)
                {
                    result.AddRange(GetSOArray(p.Copy(), level + 1));
                    continue;
                }
        ...
        internal static List<SerializedProperty> GetSerializedProperties(Object go, bool processArray)
        {
            var result = new List<SerializedProperty>();
            ...
            while (iterator.NextVisible(true))
            {
                ...
                if (processArray && iterator.isArray)
                    result.AddRange(GetSOArray(iterator.Copy(), 0));
            }
            return result;
        }
    }
