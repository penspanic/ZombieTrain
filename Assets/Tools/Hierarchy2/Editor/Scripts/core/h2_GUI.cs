...
namespace vietlabs.h2
{
    public class h2_GUI
    {
        ...
        private static GUIStyle _labelIcon;

        private static readonly Dictionary<string, int> miniLabelWCache = new Dictionary<string, int>();

        private static readonly Color LINE_COLOR_LIGHT = new Color32(128, 128, 128, 255);
        private static readonly Color LINE_COLOR_DARK = new Color32(64, 64, 64, 255);
        private static readonly Color BANNER_COLOR_LIGHT = new Color32(64, 64, 64, 255);
        private static readonly Color BANNER_COLOR_DARK = new Color32(0, 0, 0, 255);
...
//            if (tex != null)
        ...
        public static void BigLabelIcon(Rect r, string label, Texture2D icon)
        {
            if (_labelIcon == null)
            {
                _labelIcon =
                    new GUIStyle(EditorGUIUtility.isProSkin ? EditorStyles.boldLabel : EditorStyles.whiteBoldLabel);
                _labelIcon.alignment = TextAnchor.MiddleRight;
            }

            r.height = 16f;
            r.xMax -= 16f;

            if (icon != null) GUI.DrawTexture(new Rect(r.xMax, r.y, 16f, 16f), icon);

            r.xMax -= 2f;
            GUI.Label(r, label, _labelIcon);
        }
        ...
        public static bool DrawBanner(bool expand, string label, float dx = 0, float dw = 0, float lbx = 0,
            float? arrowx = null)
        {
            ...
            if (Event.current.type == EventType.repaint)
            {
                ...
                if (arrowx != null)
                {
                    rect.x += rect.width - 16f + arrowx.Value;
                    rect.width = 16f;
